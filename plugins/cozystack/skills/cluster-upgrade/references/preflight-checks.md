...
kubectl --context $CTX get nodes                     # all Ready, versions consistent
```
...
```bash
kubectl --context $CTX get packages.cozystack.io -A                       | grep -v True
kubectl --context $CTX get artifactgenerators.source.extensions.fluxcd.io -A | grep -v True
kubectl --context $CTX get externalartifacts.source.toolkit.fluxcd.io -A  | grep -v True
...
for k in helmreleases kustomizations gitrepositories helmrepositories ocirepositories buckets artifactgenerators; do
  kubectl --context $CTX get "$k" -A -o json 2>/dev/null | \
...
```bash
kubectl --context $CTX get pods -A --no-headers | awk '$4!="Running" && $4!="Completed"'
kubectl --context $CTX get tenantcontrolplane -A      # STATUS=Ready, VERSION==INSTALLED VERSION (no version drift)
kubectl --context $CTX get tenants.apps.cozystack.io -A   # READY=True
```
...
kubectl --context $CTX get node -o wide -l node-role.kubernetes.io/control-plane=
```

## etcd (if tenants use kamaji with etcd datastore)

```bash
kubectl --context $CTX -n tenant-root get hr etcd                  # Ready=True
kubectl --context $CTX -n tenant-root get sts etcd                  # 3/3 Ready
kubectl --context $CTX get datastores.kamaji.clastix.io             # endpoints resolve
# DNS resolution of etcd service from inside cluster:
kubectl --context $CTX -n tenant-root get svc etcd
```
...
```bash
kubectl --context $CTX get resourcequota -A
```
