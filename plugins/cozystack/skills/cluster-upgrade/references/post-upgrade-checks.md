...
kubectl --context $CTX -n cozy-system get configmap cozystack-version -o yaml 2>/dev/null

# 2. Full readiness (must be all-green)
bash <cozystack-repo>/hack/check-readiness.sh
kubectl --context $CTX get hr -A | grep -v True                          # empty
kubectl --context $CTX get packages.cozystack.io -A | grep -v True       # empty
kubectl --context $CTX get pods -A --no-headers | awk '$4!="Running" && $4!="Completed"'   # empty

# 3. No new suspensions introduced by upgrade
for k in helmreleases kustomizations; do
  kubectl --context $CTX get "$k" -A -o json | jq -r ".items[] | select(.spec.suspend==true) | \"$k: \(.metadata.namespace)/\(.metadata.name)\""
done

# 4. Tenant control planes reachable, versions match
kubectl --context $CTX get tenantcontrolplane -A
...
for tcp_ns_name in $(kubectl get tenantcontrolplane -A -o jsonpath='{range .items[*]}{.metadata.namespace}/{.metadata.name} {end}'); do
  ...
  kubectl --context $CTX -n "$ns" get secret "${name}-admin-kubeconfig" -o jsonpath='{.data.admin\.conf}' | base64 -d > "$tmp_kubeconfig"
